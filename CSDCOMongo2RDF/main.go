...
func CSDCORDF(session *mgo.Session) {

	csvw := session.DB("test").C("csdco")
	...
	for _, item := range csvwDocs {
		...
		newpred11, _ := rdf.NewIRI("http://opencoredata.org/id/voc/csdco/v1/date")
		...
		newobj1, _ := rdf.NewLiteral(item.LocationName)
		...
		newobj14, _ := rdf.NewLiteral(item.State_Province)
		newobj15, _ := rdf.NewLiteral(item.County_Region)
		newobj16, _ := rdf.NewLiteral(item.PI)
